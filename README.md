...
2. [Project Overview](#2-build,-load,-and-run-instructions)
3. [Execution Screenshots](#3-screenshots-of-execution)
...
Create the **rootfs-base outside the src folder** and execute the following commands:
...
Create one writable copy per container before launch, **within the src folder**:
...
# All 3 containers are required.
```

Do not commit `rootfs-base/` or `rootfs-*/` to the repository.

## From Now All these commands will be exectued within src

### Load the Kernel Module

```bash
sudo insmod monitor.ko 	# It must return nothing

# If it returns insmod: ERROR: could not insert module monitor.ko: File exists
# Then run this:

sudo rmmod monitor.ko	# remove module monitor.ko
sudo insmod monitor.ko	# insert it again                                                                                        

# Once the above commands are done follow the next 

# Verify the control device exists
ls -l /dev/container_monitor
```

### Comparing Linux CFS Scheduler with the Scheduler Implemented by us:

```bash
sudo chmod +x compare.sh
sudo ./compare.sh	# A compare module will run on its own do not interrupt, wait for it to end and return results
```

![screenshot-comparesh](Screenshots/comparesh_suite_1.png)
![screenshot-comparesh](Screenshots/comparesh_suite_2.png)
![screenshot-comparesh](Screenshots/comparesh_results_summary.png)
![screenshot-comparesh](Screenshots/comparesh_analysis.png)

### Copy Workload Binaries into the Root Filesystems

Execute all this **within the src folder**:
...
The supervisor creates its control socket at **`/tmp/mini_runtime.sock`** and stays alive in the foreground. 

**Open a second terminal for the CLI commands below.**

### Using the CLI

```bash
# Start two containers in the background
sudo ./engine start alpha ./rootfs-alpha "/cpu_hog 15" --soft-mib 48 --hard-mib 80
sudo ./engine start beta ./rootfs-beta "/io_pulse 30 100" --soft-mib 64 --hard-mib 96
...
sudo ./engine run alpha1 ./rootfs-alpha "/cpu_hog 10"

# Stop a running container
sudo ./engine stop alpha1
```
...
sudo ./engine start beta1 ./rootfs-beta1 "/memory_hog 25 5000" --soft-mib 20 --hard-mib 100
...
sudo ./engine start high_prio ./rootfs-alpha "/cpu_hog 15" --nice -15
sudo ./engine start low_prio ./rootfs-beta "/cpu_hog 15" --nice -15

sudo ./engine logs high_prio
sudo ./engine logs low_prio
...
# OR the following in some other Terminal
```bash
...
![Screenshot-1-Multi-Container](Screenshots/Terminal_1_Start.png)
...
> CPU HOG:
![Screenshot-2-alpha](Screenshots/rootfs-alpha.png)

![Screenshot-2-alpha-proof](Screenshots/alpha_started_proof.png)

> I/O PULSE:
![Screenshot-2-beta](Screenshots/beta_started.png)

![Screenshot-2-alpha-beta](Screenshots/alpha-beta-exited.png)

![Screenshot-2-alpha-beta](Screenshots/alpha_beta_unregistered.png)

> MEMORY HOG:
![Screenshot-2-memory-hog](Screenshots/memory_hog.png)
![Screenshot-2-memory-hog-ps](Screenshots/memory_hog_ps.png)
...
![screenshot-3-logs_alpha](Screenshots/logs_alpha.png)

![screenshot-3-logs_beta](Screenshots/logs_beta.png)
...
![screenshot-4-CLI_sent](Screenshots/CLI_sent.png)

![screenshot-4-CLI_Terminal_1](Screenshots/CLI_Terminal_1.png)

---

### Screenshot of Container Monitor logs

![screenshot-container_monitor](Screenshots/container_monitor.png)
...
![screenshot-7-scheduling](Screenshots/high_prio-low_prio.png)

> logs of those low and high priority processes:

![screenshot-7-scheduling](Screenshots/low_prio_logs.png)
![screenshot-7-scheduling](Screenshots/high_prio_logs.png)

> Exit of low priority and high priority processses:
![screenshot-7-scheduling](Screenshots/low-prio_high-prio_monitor.png)
...
> Alpha-Beta Exited:
![screenshot-8-teardown](Screenshots/alpha_beta_reaped.png)

> Remove monitor.ko:
![screenshot-8-teardown](Screenshots/container_monitor_unloaded.png)

> SIGINT on the Terminal-1:
![screenshot-8-teardown](Screenshots/Terminal_1_Smooth_Exit.png)

> List of processes running before and after teardown, to verify zombies do not remain:
![screenshot-8-teardown](Screenshots/ps_aux.png)
